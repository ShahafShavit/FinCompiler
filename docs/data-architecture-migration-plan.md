...
**Last updated:** 2026-04-11 — **MIG-D4 signed:** from **2026-04-11**, **`config.ledger_db_file` (SQLite) is the only authoritative ledger** for this project; other ledger representations (**`compiled.csv` as truth**, **`fingerprint_db.csv`**, treating Sheets or raw CSV as authoritative) are **deprecated** — see tracker MIG-D4 notes. **Repo layout:** single `pipeline/ledger.py` for all ledger DB operations (see Section 1.4). **Transactions pipeline (2026-04-11):** raw workbooks → in-memory normalize → `dedupe_import_batch_by_fingerprint` → `upsert_compiled_dataframe_to_ledger` — **no** full-ledger pandas merge, **no** `compiled.csv` write in normal runs. **Holdings pipeline:** raw workbooks → in-memory wide merge → `upsert_holdings_wide_to_ledger`. Optional **`PIPELINE_DEBUG_DUMP=1`** writes legacy cleaned CSVs under `data/pipeline/*/clean` for debugging only. Legacy CSV columns **מזהה עסקה** / **תאריך עדכון** vs ledger **fingerprint** / **ingested_at** — see `docs/ledger-merge-ownership.md`, `schema/ledger/README.md`, `pipeline/ingested_at_rules.py`. **MIG-E3:** `fingerprint_db.csv` is not read or written when `config.ledger_db_file` exists on disk; legacy CSV-only runs (no ledger file) may still exist in code for tests — **not** supported as a production workflow after MIG-D4.  
...
| MIG-A3 | A | Test baseline run | Done | 2026-04-11: `python -m unittest discover -s tests -p "test_*.py"` — **52** tests, exit 0, repo root (includes Phase G policy tests, `test_ingested_at_rules`, compile upsert tests, heatmap tests) |
...
| MIG-C4 | C | Static mappings + holdings tables (one DB; no separate fingerprint_metadata) | Done | Same single DB file: `store` / `store_category`, `holdings_balance` in `full_schema.sql` |
| MIG-D1 | D | Import: ledger | Done | **`web_totals` / `import_web_totals_to_ledger` removed** (2026). Ledger rows enter only via **`upsert_compiled_dataframe_to_ledger`** (pipeline) or other **explicit** `pipeline.ledger` helpers — no CSV bootstrap from `web/data`. |
| MIG-D2 | D | Import: static mappings (stores → SQLite) | Done | `pipeline/ledger.py` (`import_stores_to_ledger`, `sync_stores_to_ledger_from_dataframe`, etc.) — `stores_to_categories.csv` → `store` / `store_category`; full replace clears mapping tables; null/empty `store_name` rows dropped. **`fingerprint_db.csv` is not imported** — redundant with **`ledger_transaction.קטגוריה`** (Section 5). |
| MIG-D3 | D | Parity report | Done | Historical: parity helpers tied to web_totals import **removed** with that path; use **`audit_ledger_constraints`** / tests for DB checks. |
...
| MIG-E2 | E | Compiler integration → SQLite | Done | Superseded by **MIG-E5** for production: transactions no longer use `Compiler` + full-ledger merge; see E5. Legacy `Compiler` remains for CSV-only tests (`ledger_db=None`). |
...
| MIG-E5 | E | Deprecate pipeline CSV staging; direct upsert | Done | **2026-04-11:** `ingest_transactions_to_ledger` / `compile_holdings_main` read raw `*.xls*` only; `data/pipeline/*/clean/*.csv` and `compiled.csv` / `holdings.csv` are **not** written in normal runs. Batch dedupe: `dedupe_import_batch_by_fingerprint` before `executemany`. Debug mirror: `PIPELINE_DEBUG_DUMP=1`. |
| MIG-E4 | E | Divergence detection stub | Not started | |
| MIG-F1 | F | Migration for timestamps | Done | `ingested_at` / timestamp columns in `schema/ledger/full_schema.sql`; `pipeline/ingested_at_rules.py` (`ingested_at_for_new_ledger_row`); covered by `tests/test_ingested_at_rules.py` |
...
**Configuration (`config.py`).** Runtime paths are centralized: `data/export/compiled/compiled.csv` is a **legacy filename** (not written by the default pipeline after **MIG-E5**); `data/static/` holds `stores_to_categories.csv` and (legacy, **CSV-only** when no ledger file) **`fingerprint_db.csv`** — redundant vs **`ledger_transaction.קטגוריה`** when the ledger exists (**MIG-E3**). **`web/data/`** may hold generated HTML/assets; **heatmap reads `ledger.sqlite`**. `FINANCE_WORKSPACE_ROOT` isolates data for tests or experiments. **Holdings** desktop tab uses a calendar-year suffix by default; **Totals** push uses **`FINANCE_TOTALS_SHEET_NAME`** / `desktop_totals_sheet_name()` (single all-time tab).

**Ledger module (`pipeline/ledger.py`).** All SQLite ledger operations live in **one** module. It includes: **`migrate_ledger_db`** (hand-rolled steps + baseline `schema/ledger/full_schema.sql`); **constraint audit** (`audit_ledger_constraints`, `format_report`); **dataframe I/O** (`load_transactions_dataframe_from_ledger`, `export_transactions_dataframe_to_csv`, store/backup loaders); **category update** (`update_category_by_fingerprint`); **fingerprint NULL backfill** (`backfill_null_fingerprints`, etc.); shared row helpers `_normalize_date_text` / `_float_col` / `_text_or_none`; **static store** (`import_stores_to_ledger`, `sync_stores_to_ledger_from_dataframe`, CSV loaders); **compile upsert** (`upsert_compiled_dataframe_to_ledger`, **`dedupe_import_batch_by_fingerprint`**). Call sites import from **`pipeline.ledger`** only. **`pipeline/folder_tracking.py`** was removed (unused `FolderTracker`; SSE logger list in `web_control` updated accordingly).

**Pipeline entrypoints.** `pipeline.ingest_transactions_to_ledger` / `pipeline.compile_transactions_main` and `pipeline.compile_holdings_main` in `pipeline/__init__.py` upsert **directly into SQLite** from raw workbooks (no `Compiler` merge for transactions; legacy `Compiler` remains for CSV-only tests). `apps/pipeline_cli.py` exposes CLI commands; `apps/qt_main.py` wires PyQt actions; `web_control/jobs.py` runs pipeline actions from the local control server. **SQLite:** `config.ledger_db_file` and `pipeline.ledger.migrate_ledger_db` create/upgrade the ledger DB from `schema/ledger/full_schema.sql`. **Optional** `PIPELINE_DEBUG_DUMP=1` writes legacy cleaned CSVs under `data/pipeline/*/clean`. **Categorization** in PyQt and web control uses `CategorizeFile(ledger_db_path=...)` against that DB. **Heatmap** (`web_control/heatmap`) loads transactions from **`ledger.sqlite`** via `load_transactions_dataframe_from_ledger`. **MIG-D4** signed **2026-04-11:** SQLite ledger is **declared canonical**; CSV/Sheets-as-truth workflows **deprecated** for operator use (see Section 0.3 tracker).
...
**Tests.** `tests/` contains `unittest`-style modules (for example `test_workspace_config.py`, `test_pipeline_date_roundtrip.py`, `test_categorization_logic.py`, `test_ledger_migrate.py`, `test_static_store_import.py`, `test_ledger_compile_upsert.py`, `test_ingested_at_rules.py`, `test_phase_g_sheets_policy.py`, `test_heatmap_category_mean.py` — most exercising **`pipeline.ledger`** or compile/timestamp rules. **pytest is not listed in `requirements.txt`**; verification should use **`python -m unittest discover`** unless the project adds pytest later. **Current baseline:** **52** tests passing from repo root (`python -m unittest discover -s tests -p "test_*.py"`).

**Google Sheets.** **Push only:** preview/compare and push from local holdings CSV + SQLite ledger export for Totals. **No pull** in code (removed `update_local`, `pull_desktop_sync_from_cloud`, web pull, PyQt pull). **Heatmap** uses **`ledger.sqlite`** directly.
...
The canonical bootstrap script is **`schema/ledger/full_schema.sql`**; **`schema/ledger/README.md`** explains ISO dates, `STRICT`, and local-time trigger defaults. The DDL defines **`ledger_transaction`** (including **`fingerprint`**, **`notes`**, evaluation Section 13.9 timestamps; **no** **`מזהה עסקה`** row-hash column), **`store`** / **`store_category`** (with static-store enforcement triggers), **`holdings_balance`**, and timestamp triggers. There is **no** `fingerprint_metadata` table — categories live on the ledger row. **One-shot imports** (via `pipeline/ledger.py` static-store helpers, `pipeline/holdings_csv_import`, or other **explicit** ledger loaders — **not** from `web_totals.csv`) can fill the ledger (MIG-D1), static mappings (MIG-D2), and holdings. **Ongoing compile** (Phase E) **writes the ledger** via `upsert_compiled_dataframe_to_ledger`. **MIG-D4** (**2026-04-11**) records the formal **canonical SQLite / return-to-service** decision (Section 0.3).
...
3. **Critical paths to verify inside the copy:** `data/export/compiled/` (especially `compiled.csv` and `holdings.csv` if used), `data/static/` (`stores_to_categories.csv`, and **`fingerprint_db.csv`** if your pipeline still produces it — **not** imported to SQLite, Section 5), **`data/ledger.sqlite`** (or `config.ledger_db_file`) for canonical ledger + mappings, and `web/data/` for generated web assets (**heatmap reads the SQLite ledger**).
...
**Goal.** Perform **one-shot import** of existing **`compiled.csv`**, **`stores_to_categories.csv`**, and **holdings** as applicable into SQLite, then **stop using CSV as the authoritative ledger** as soon as the implementing change set lands — **no stabilization phase** where the pipeline keeps writing both or keeps `compiled.csv` as source of truth. **`fingerprint_db.csv` is out of scope for import** — redundant with **`ledger_transaction`** category columns (**Section 5**); **MIG-E3** stops writing it.
...
| ID | Title | Description | Dependencies | Owner | Acceptance criteria | Risk |
|----|--------|-------------|--------------|-------|---------------------|------|
| MIG-D1 | Import: ledger (API) | Deterministic ledger writes via **`pipeline.ledger`** (**`upsert_compiled_dataframe_to_ledger`**, explicit static/holdings loaders): row order preserved or documented; **fingerprints** unique; failed duplicates **reported**, not silent drop. **`web_totals` import removed.** | Phase C | Implementer | Import completes; log prints row counts; **UNIQUE** fingerprint holds. | High |
| MIG-D2 | Import: static mappings | Load **`store`** / **`store_category`** from `stores_to_categories.csv` via `pipeline/ledger` (`import_stores_to_ledger`, etc.). **`fingerprint_db.csv`:** **no** import — redundant with **`ledger_transaction.קטגוריה`** (Section 5). Validate row counts vs CSV after import. | Phase C | Implementer | Store row counts match deduped CSV; triggers satisfied (static vs dynamic stores). | Medium |
...
**Static mappings import (MIG-D2 slice).** After `migrate_ledger_db`, call **`import_stores_to_ledger`** from **`pipeline.ledger`** (or sync via **`sync_stores_to_ledger_from_dataframe`**) to populate **`store`** and **`store_category`**. This is independent of the ledger row import (MIG-D1) but uses the same SQLite file.
...
| 5 | **Holdings vs ledger** | **Same SQLite file** — one database (`ledger_transaction` + **`store`** / **`store_category`** + **`holdings_balance`**, etc.), consistent with a **single all-time ledger** model. |
...
- **Tasks:** Add `ledger.sqlite` path; **hand-rolled** migration runner (**`migrate_ledger_db`**); align with **`schema/ledger/full_schema.sql`** (`ledger_transaction`, `store`/`store_category`, `holdings_balance`); **UNIQUE(fingerprint)** on ledger; gitignore DB files.
...
| 2026-04-11 | **MIG-E5 Done:** In-memory pipeline — `ingest_transactions_to_ledger`, holdings raw→`upsert_holdings_wide_to_ledger`; `dedupe_import_batch_by_fingerprint`; `PIPELINE_DEBUG_DUMP` for optional clean CSV mirrors; **MIG-E2** notes superseded for production. `integrations/google_sheets.push_monthly_look` reads ledger when DB exists. |
| 2026-04-11 | **`web_totals` fully deprecated:** `import_web_totals_to_ledger`, parity helpers, and `web_totals` tests/fixtures removed; `ingested_at_rules` is **`ingested_at_for_new_ledger_row` only**; config/docs scrubbed. Unittest baseline **52**. |
