...
def ingested_at_for_new_ledger_row() -> str:
    """``YYYY-MM-DD`` for a row inserted by the live pipeline (local ``date.today()``)."""
...
def upsert_compiled_dataframe_to_ledger(
    ...
    New rows get ``ingested_at`` = local insert date (:func:`ingested_at_for_new_ledger_row`).
...
# --- Installment statement_month backfill (ex-installment_statement_months) ---
...
def _exclude_parion_installment(detail: str) -> bool:
    return "פרעון תשלום" in detail


def _month_after_taarich_installment(taarich: str) -> str | None:
    try:
...
def _add_months_ym(ym: str, n: int) -> str:
...
def _cluster_installment_by_amount(rows: list[_InstallmentRow], max_span: float) -> list[list[_InstallmentRow]]:
    if not rows:
        return []
    s = sorted(rows, key=lambda r: r.bh)
    out: list[list[_InstallmentRow]] = []
    i = 0
    n = len(s)
    while i < n:
        j = i
...
def _installment_load_candidates(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    cur = conn.execute(
        f"""
...
def _installment_build_updates(
    ...
    parsed: list[_InstallmentRow] = []
    for r in rows_raw:
        did = r["id"]
        detail = str(r["פירוט נוסף"])
        if _exclude_parion_installment(detail):
            continue
        inst = _parse_installment_detail(detail)
        if not inst:
            continue
        x, y = inst
        ...
        parsed.append(_InstallmentRow(id=did, taarich=ta, bh=bh, source=src, detail=detail, x=x, y=y))

    buckets: dict[tuple[str | None, str, int], list[_InstallmentRow]] = {}
    for row in parsed:
    ...
    for key, bucket in buckets.items():
        src, ta, y = key
        base = _month_after_taarich_installment(ta)
        if base is None:
            warnings.append(f"bucket source={src!r} taarich={ta} y={y}: invalid תאריך")
            continue

        for cluster in _cluster_installment_by_amount(bucket, amount_tol):
            by_x: dict[int, _InstallmentRow] = {}
            dup = False
            for row in cluster:
                if row.x in by_x:
                    warnings.append(
            ...
            for row in cluster:
                sm = _add_months_ym(base, row.x - 1)
                proposed.append(
                    {
                        "id": row.id,
...
def run_installment_statement_month_fill(
    ...
    *,
    ...
    rows_updated = 0
    try:
        raw = _installment_load_candidates(conn)
        proposed, warnings, cluster_stats = _installment_build_updates(raw, amount_tol)
